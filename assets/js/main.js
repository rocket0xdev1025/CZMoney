...
document.querySelectorAll("[data-key]").forEach((el, i) => {
  el.addEventListener("click", function (e) {
    ...
    if (el.getAttribute("data-key") === key) {
      el.querySelector("[data-acc-title]").classList.add(
        "pt-6",
        "sm:pt-12",
        "pb-6",
        "text-primary"
      ...
      el.querySelector("[data-acc-body]").classList.remove(
        "h-0",
        "overflow-hidden"
      );
...
function AccordionCheck() {
  document.querySelectorAll("[data-key]").forEach((el, i) => {
    el.querySelector("[data-acc-title]").classList.remove(
      "pt-6",
      "sm:pt-12",
      "pb-6",
      "text-primary"
...
document.querySelectorAll("[data-sideToggle]").forEach((el, i) => {
  el.addEventListener("click", function (e) {
    document
      .querySelector('[data-menu="mobile"]')
...
window.addEventListener("scroll", function () {
  if (window.pageYOffset > 50) {
    ...
    document
      ...
      .classList.add("backdrop-blur-sm", "!bg-body/80");
  } else {
    document.querySelectorAll("[data-desktop-navmenu] li a").forEach((el) => {
    ...
    document
      .querySelector("[data-navbar]")
      .classList.remove("backdrop-blur", "!bg-body/80");
  }
});
